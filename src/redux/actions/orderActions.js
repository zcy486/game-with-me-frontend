  ...
  return async (dispatch) => {
    try {
      ...
      interval = setInterval(
        updatedOrder(gamerId, companionId, timeout, currentBalance),
        3000
      );
    ...
    try {
      if (window.localStorage["order"]) {
        ...
        if (updatedOrder.orderStatus === "Created") {
          alert(
            "Your order has been automatically cancelled because companion didn't confirmed."
          );
    ...
    if (window.localStorage["order"]) {
      ...
      try {
        ...
        if (updatedOrder.orderStatus === "Confirmed") {
          ...
          await UserService.recordPayment(
            order.gamerId,
            "Paid",
            order.orderPrice,
            "self-account",
