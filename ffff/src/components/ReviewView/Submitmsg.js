...
function SubmitReview({review, handleSubmit, history}) {
  debugger;
  //deconstruct review based on Review Model
  const {orderId, companionId, gamerId} = review;
  const classes = useStyles();
  //construct basic review data based on Review Model
  const [_id, setId] = useState(review._id); //review _id
  const [star, setStar] = useState(review.star); //review star
  ...
  return (
    <div>
      ...
      <div>  
        <FormGroup row>
          {LABEL_ENUMS.map((labelText) => {
            ...
            const CustomCheckBox = (labelText === "rude")? "PurpleCheckbox": "BlueCheckbox";
            const CustomIcon = (labelText === "rude")? "SentimentDissatisfiedIcon": "SentimentSatisfiedAltIcon";
            return <FormControlLabel
              control={<CustomCheckBox icon={<CustomIcon />} checked={ifChecked} onChange={handleLabling} name={labelText} />}
              label={labelText}
            />
          })}
          {/* <FormControlLabel
      ...
      <div>
        ...
        //}
          onClick={() => {
            handleSubmit(_id, star, label, reviewText, orderId, companionId, gamerId);
            handleClickOpen();
          }}>
          Submit
        </Button>
