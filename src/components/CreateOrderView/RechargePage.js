...
function RechargePage(props) {
  ...
  const onApprove = (data, actions) => {
    return actions.order.capture().then(function (details) {
      ...
      const newBalance =
        ...
        Number(details.purchase_units[0].items[0].quantity);
      props.handleRecharge(
        newBalance,
        Number(details.purchase_units[0].items[0].quantity),
  ...
  return (
    <Dialog
      ...
      maxWidth={"lg"}
    >
      <DialogTitle id="dialog-title">{"Recharge Page"}</DialogTitle>
      <DialogContent dividers={true}>
        ...
        {invalidValue ? (
          ...
          <Grid>
            <DialogContentText id="dialog-description">
              You will have to pay {payValue} EURO to get {Number(value)}{" "}
              ecoins.
            </DialogContentText>
